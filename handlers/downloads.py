...
async def download_book(call: types.CallbackQuery, callback_data: dict):
    ...
    wait = await call.message.answer(f'Ожидайте, начинаю скачивать книгу 🙃')

    response = await get_tempfile(url)
    try:
        res_to_bytesio = BytesIO(response.read())  # конвентируем книгу в байты для отправки
    except AttributeError:
        return await wait.edit_text('Упс! Возникли небольшие неполадки на сервере 😲\n'
    ...
    try:
        ...
        await wait.edit_text(f'Не могу отправить файл т.к. в телеграме есть огранчиния по весу файлов😔\n'
